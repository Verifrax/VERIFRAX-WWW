...
const URL_ROOT = process.env.URL_ROOT || "https://www.verifrax.net";
const OUT = process.env.OUT || fs.mkdtempSync(path.join(os.tmpdir(), "verifrax-live-runtime-doctrine-"));
...
function fetchText(url) {
  return new Promise((resolve, reject) => {
    https.get(url, res => {
      ...
      this.destroy(new Error(`timeout for ${url}`));
    });
  });
}

function selectedApi(api) {
  if (!api) return null;
  return {
...
(async () => {
  const assetUrl = `${URL_ROOT}/assets/observatory-webgl-runtime.js?v=${Date.now()}`;
  const asset = await fetchText(assetUrl);
  fs.writeFileSync(path.join(OUT, "observatory-webgl-runtime.js"), asset);
  ...
  const missingAssetMarkers = requiredAssetMarkers.filter(marker => !asset.includes(marker));
  if (missingAssetMarkers.length) {
    console.error(JSON.stringify({
      ok: false,
      doctrine: "live_runtime_doctrine_deployed_watchdog",
      failure: "missing_live_asset_markers",
      assetUrl,
      missingAssetMarkers
    }, null, 2));
    process.exit(1);
  ...
  for (let i = 0; i < 240; i++) {
    facts = await page.evaluate(() => {
      ...
      return {
        ...
        compositionApi: pick("VCO_TERMINAL_FOREGROUND_COMPOSITION_GOVERNOR_API"),
        compositionAliasApi: pick("VCO_TERMINAL_FOREGROUND_COMPOSITION_API"),
        hardOcclusionApi: pick("VCO_TERMINAL_HARD_FOREGROUND_OCCLUSION_API"),
        absoluteApi: pick("VCO_TERMINAL_ABSOLUTE_VISUAL_LOCK_API"),
        noAtomApi: pick("VCO_TERMINAL_NO_ATOM_ORBIT_API")
      };
    });
  ...
  const screenshot = path.join(OUT, "live-runtime-doctrine-watchdog.png");
  ...
  fs.writeFileSync(path.join(OUT, "live-runtime-doctrine-watchdog.json"), JSON.stringify(proof, null, 2));
